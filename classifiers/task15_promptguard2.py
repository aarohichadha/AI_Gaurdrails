...
Runs Meta's Llama Prompt Guard 2 over the corpus and reports precision,
recall, F1, attack recall, false positives and latency - with a focus on
**contextual attacks**: attacks that carry no injection syntax at all and are
only harmful because of *who* is asking, *where* the data goes and *whether*
it was authorized.

Prompt Guard only ever sees text. By default it classifies the untrusted
channel - email subject + body - which is how Meta intends it to be
deployed (screen third-party content before it reaches the agent).

    python classifiers/task15_promptguard2.py                     # 86M, full corpus
    python classifiers/task15_promptguard2.py --model pg2-22m
    python classifiers/task15_promptguard2.py --input full        # add context + action
    python classifiers/task15_promptguard2.py --reuse             # re-analyse cached scores
    python classifiers/task15_promptguard2.py --backend mock --limit 200   # dry run, no model
"""
...
#
# Prompt Guard is trained to recognise *injection syntax*: "ignore previous
# instructions", role-play jailbreaks, embedded commands. Several categories
# in this corpus contain none of that - the text reads like an ordinary
# business request and is only an attack in context. The grouping below is
# ours (not the corpus's) and is what "how well does it detect contextual
# attacks?" is answered against.
# ---------------------------------------------------------------------------
...
def select_actions(view: str, split: Optional[str], limit: Optional[int], seed: int) -> List[Action]:
    ...
    if limit and limit < len(actions):
        # Deterministic shuffle so a small run still mixes every sheet.
...
def as_guardrail(scores: Dict[str, float], threshold: float):
    ...
    def guardrail(action: Action) -> GuardrailResult:
        ...
        if score >= threshold:
            result.apply(Decision.BLOCK, "PG2", f"attack score {score:.3f} >= {threshold}")
        else:
...
def recall_at_fpr(actions, scores, budget: float) -> Optional[dict]:
    ...
    The threshold is chosen *on the evaluation data itself*, so this is an
    optimistic upper bound on what tuning could buy, not a deployable number.
    """
    ...
    # Threshold just above the (allowed_fp+1)-th highest benign score.
    cutoff = benign[allowed_fp] if allowed_fp < len(benign) else -1.0
    # Smallest float strictly above the cutoff. Not rounded: rounding could
    # land back on the cutoff and silently admit one more false positive.
...
def print_report(name, run, actions, scores, threshold, latency, extra) -> None:
    m = run.metrics
    print("\n" + "=" * 97)
    print(f"TASK 15 - PROMPT GUARD 2 BASELINE   ({name})")
    print("=" * 97)
...
def main(argv=None) -> dict:
    ...
    run_name = f"task15_{model_label}_{args.input_mode}_{args.view}" + (f"_{args.split}" if args.split else "")
    log_path = RESULTS_DIR / f"{run_name}.jsonl"
    print(f"Task 15 - {len(actions)} records | model {model_label} | input {args.input_mode} | view {args.view}")
    ...
    run = evaluate(as_guardrail(by_id, args.threshold), actions, f"pg2:{model_label}", args.view)
